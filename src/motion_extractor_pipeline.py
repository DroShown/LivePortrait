...
from .utils.helper import mkdir, basename, dct2device, is_video, is_directory, is_template, remove_suffix, is_image, is_square_video, is_square_image, calc_motion_multiplier
...
class ExtractMotionPipeline(object):
    ...
    def extract_motion_features(self, path:str, saving_path:str):
        """
        This function is used to extract motion features from a single image or a dir.
        Motion features will be saved as a .pkl file.
        """
        # for convenience
        ...
        if osp.exists(path):
            if is_image(path):
                # load from image file, AND make motion template
                output_fps = 25
                log(f"Load driving image from: {path}")
                driving_rgb = load_image_rgb(path)
                driving_rgb = resize_to_limit(driving_rgb, inf_cfg.source_max_dim, inf_cfg.source_division)
                driving_rgb_lst = [driving_rgb]
            if is_video(path):
                # load from video file, AND make motion template
                output_fps = int(get_fps(path))
                log(f"Load driving video from: {path}, FPS is {output_fps}")
                driving_rgb_lst = load_video(path)
            elif is_directory(path):
                # load from image directory, AND make motion template
                output_fps = 25
                log(f"Load driving video from: {path}, FPS is {output_fps}")
                driving_rgb_lst = load_image_rgb_lst(path)
        else:
        ...
        if inf_cfg.flag_crop_driving_video or (not is_square_image(driving_rgb_lst[0])): #flag_crop_driving_video 默认 False
            ...
            if len(ret_d["frame_crop_lst"]) is not n_frames:
                log(f"n_frames({n_frames}) doesn't equal len(frame crop list)({len(ret_d['frame_crop_lst'])})!")
                n_frames = min(n_frames, len(ret_d["frame_crop_lst"]))

            driving_rgb_crop_lst, driving_lmk_crop_lst,bbox_lst = ret_d['frame_crop_lst'], ret_d['lmk_crop_lst'], ret_d['bbox_lst']
        ...
        driving_template_dct['bbox_lst'] = bbox_lst
        dump(saving_path, driving_template_dct)
        log(f"Dump motion template to {saving_path}")
