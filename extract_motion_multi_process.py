...
def main(args):
    ...
    extractor_pipeline.extract_motion_features(str(args_lst[0][0]), str(args_lst[0][1]))


    start = time.time()
    num_workers = 2
    with Pool(processes=num_workers, initializer=worker_init, initargs=(extractor_pipeline,)) as pool:
        results = list(tqdm(pool.imap(process_task, args_lst), total=len(args_lst), desc="Processing motion features"))
    end = time.time()
    print(end - start)
